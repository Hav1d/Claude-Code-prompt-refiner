...
1. **已保存配置**：`~/.prompt-refiner/config.json` → 活跃 Provider 的 `api_key`
2. **已保存配置**：`~/.prompt-refiner/config.json` → 活跃 Provider 的 `auth_token`
3. **环境变量**：`ANTHROPIC_API_KEY`
4. **环境变量**：`ANTHROPIC_AUTH_TOKEN`（Bearer token）
5. **回退**：`~/.claude/config.json` → `primaryApiKey`（自动检测）
...
2. **用户配置**：`~/.prompt-refiner/config.json`
3. **项目配置**：`./prompt-config.json`
4. **显式路径**：`--config` 参数
...
运行 `/plugin install prompt-refiner@Hav1d` 即可。插件会自动：
- 注册 `SessionStart` Hook 来引导 Python 依赖
